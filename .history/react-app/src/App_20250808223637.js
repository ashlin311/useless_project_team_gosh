...
function App() {
  ...
  useEffect(() => {
    ...
    // Use hardcoded token for now (remove this in production)
    const hardcodedToken = 'BQBfsOwv4NqKBWdF3DisqkLbyPGFeTBSi4joEb28QfRTpfG_F4fUaVDZ7aLpxdaFyyOaz3XZuVDc2SNHStKnN45IfXu_35kXhINs8T2mlLdQVP_qWbvFdKoS2Wm6GFXzQEVwPnuG5HaLo23k5_jbd9TOkAkdPIyxUrDrffzn9reJL6IsXtmxCJPZRnkX2s5OZJSuIgDfylReooxzi6GBbL2treULeaVymysYbsqW8SCZIrzxe8DsR3XU2RXTXpBmQqO0_KIDU5MU7AVR8Fs7ysJJS0ZnB9aw_lVyGvI-fsq4Orbuq-QEAyPEAYFr';
    ...
    // Use hardcoded token if no stored token
    if (!token) {
          ...
          <>
            ...
            <div className="login-section">
              ...
              <button className="spotify-login-btn" onClick={() => {
                ...
                setAccessToken(token);
                fetchUserProfile(token);
              }} style={{marginTop: '10px', background: '#1ed760'}}>
                <span className="spotify-icon">⚡</span>
                Use Test Token (Skip OAuth)
              </button>
