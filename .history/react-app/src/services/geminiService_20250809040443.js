...
export const generateAudioRoast = async (audioFile, actorId, severity = 'funny', spotifyData = null, mimeType = 'audio/webm') => {
  try {
    ...
    console.log('🎵 Audio roast params:', {
      actorId,
      severity,
      mimeType,
      audioDataLength: audioFile?.length || 0,
      audioDataPreview: audioFile?.slice(0, 50) || 'no data'
    });
...
`;
    ...
    const result = await model.generateContent([
      systemPrompt,
      {
        inlineData: {
          data: audioFile,
          mimeType: mimeType // Use the actual MIME type
        }
      }
    ]);
    ...
    try {
      ...
      console.error('❌ Raw response was:', roastResult);
      
      // Try to extract JSON from response if it's wrapped in other text
      const jsonMatch = roastResult.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
          const extractedJson = JSON.parse(jsonMatch[0]);
          return {
            success: true,
            audioRoast: extractedJson
          };
        } catch (extractError) {
    ...
    if (error.message?.includes('PERMISSION_DENIED') || error.message?.includes('401')) {
      return {
        ...
        fallbackRoast: `Ayyo, API key issue! Please check the configuration and try again! 🔑`
      };
    }
    ...
    if (error.message?.includes('QUOTA_EXCEEDED') || error.message?.includes('429')) {
      return {
        ...
        fallbackRoast: `Ayyo, too many requests! Please wait a moment and try again! ⏳`
      };
    }
    ...
    if (error.message?.includes('INVALID_ARGUMENT') || error.message?.includes('400')) {
      return {
        ...
        fallbackRoast: `Ayyo, audio format issue! Try a different audio file format! 🎵`
      };
    }
