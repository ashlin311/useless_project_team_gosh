...
function App() {
  ...
  const handleSpotifyLogin = () => {
    ...
    // Show instructions
    alert(`✅ LOGIN STEPS:
1. Complete Spotify authorization in the popup window
2. After authorizing, you'll be on a Spotify documentation page
3. Look at the URL bar and find "access_token="
4. Copy ONLY the token part (starts with BQ...)
5. Come back here and paste it when prompted

Example: If URL shows "access_token=BQAbc123...", copy "BQAbc123..."`);
    
    // Wait a bit then prompt for token
    setTimeout(() => {
      const token = prompt('Paste your access_token here (starts with BQ...):');
      if (token && token.trim() && token.startsWith('BQ')) {
        window.localStorage.setItem('spotify_token', token.trim());
        setAccessToken(token.trim());
        fetchUserProfile(token.trim());
        if (popup) popup.close();
      } else if (token) {
        alert('Invalid token format. Please copy only the token part that starts with "BQ"');
      }
    }, 3000);
  };  return (
    <div className="App">
